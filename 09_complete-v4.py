"""Full program version 4 of Chinese and Japanese quiz
...
# This function scores the answer and returns a suitable
...
def score_question(answer, number):
    if answer == number[1] or answer == str(number[2]):
        ...
        return "{} + {} points\n"\
            .format(make_green("✔ ") +
                    make_bold(get_random_response(correct_responses)),
                    POINT_VALUE)
    else:
        return "{}\nCorrect answer is: {}\n"\
            .format(make_red("❌ ") +
                    make_bold(get_random_response(incorrect_responses)),
                    make_bold(number[1]))
...
while keep_playing:
    ...
    print("+------------------------------------------+")
    quiz_type = input(
        "Would you like to play a Chinese quiz or a Japanese quiz? ")

    while quiz_numbers is None:
    ...
    for number in shuffled_numbers:
        answer = input("What number does '{}' stand for in English? "
                       .format(make_bold(number[0])))
        print(score_question(answer, number))

    # Game finished
    print("Your total score was: {} out of {}"
          .format(score_counter, number_of_questions * POINT_VALUE))
    print(judge_score())
